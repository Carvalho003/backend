-- PostgreSQL version of view dashboard financeiro
-- Converted from MySQL syntax

DROP VIEW IF EXISTS v_dash_kpi_mes CASCADE;
DROP VIEW IF EXISTS v_dash_vendas_categoria CASCADE;
DROP VIEW IF EXISTS v_dash_despesas_categoria CASCADE;
DROP VIEW IF EXISTS v_dash_kpi_a_pagar CASCADE;
DROP VIEW IF EXISTS v_dash_proximos_pagamentos CASCADE;
...
WITH DadosMensais AS (
    SELECT
        (date_trunc('month', M.data_pagamento))::date AS MES_REFERENCIA_FORMATADA,
    ...
    GROUP BY date_trunc('month', M.data_pagamento), M.tipo
),
...
SELECT
    ...
    T.MES_REFERENCIA_FORMATADA AS mes_referencia
